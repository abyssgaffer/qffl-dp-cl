...
data_weights = torch.load(f'../result/data/{NAME_MODELS}_data_weights')  # python list
    ...
    for pos in pos_list:
        for eps in ([None] if mech == 'none' else epsilons):
            ...
            if mech == 'none':
                ...
                if mech in ('gaussian', 'quantum'):
                    # 每节点 (ε/H, δ/H)-LDP，L2敏感度 = 2*tau
                    eps_node = eps / H
                    delta_node = delta / H
                    # σ = sqrt(2 ln(1.25/δ_node)) * (2*tau) / eps_node
                    scale_y = math.sqrt(2 * math.log(1.25 / delta_node)) * (2.0 * tau) / eps_node
                    # e 通道不用的话随便占位
                    scale_e = None

                elif mech == 'laplace':
                    # 每节点 ε/H-LDP，L1敏感度 = 2*tau*sqrt(d)
                    eps_node = eps / H
                    # b = (2*tau*sqrt(d)) / (ε/H) = 2*tau*sqrt(d)*H/ε
                    scale_y = (2.0 * tau * math.sqrt(d)) / eps_node
                    scale_e = None
