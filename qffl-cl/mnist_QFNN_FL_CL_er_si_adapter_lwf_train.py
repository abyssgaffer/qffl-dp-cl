...
# ======================== 每个客户端 ========================
for i in range(node):
    model = Qfnn(DEVICE, use_adapter=True).to(DEVICE)
    ...
    for epoch in range(EPOCH):
        ...
        for x, y in tqdm(train_loader):
            ...
            for name, param in model.named_parameters():
                si_loss += torch.sum(omega[name] * (param - prev_params[name]) ** 2)
            total_loss = ce_loss + ALPHA_LWF * distil_loss + LAMBDA_SI * si_loss
            total_loss.backward()

            # 累积W
            for name, param in model.named_parameters():
                if param.grad is not None:
                    W[name] += -param.grad * (param.detach() - prev_params[name])
            optimizer.step()
        ...
        for name, param in model.named_parameters():
            ...
            omega[name] += W[name] / (delta ** 2 + 1e-6)
            W[name].zero_()
